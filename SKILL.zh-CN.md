...
算法：
1. 递归列出仓库中所有已有笔记（排除 `_index.md`、`_tags.md`、`_graph.md`、`_directory-map.md`、`CONVENTIONS.md` 和 `templates/`）
2. 从新笔记内容中提取至多 5 个候选链接目标：
   a. 新笔记正文中已有的 `[[...]]` 引用
   b. 摘要或正文中与已有笔记标题匹配的主题（不区分大小写）
   c. 新笔记与已有笔记共有的标签
3. 对每个候选：
   a. 阅读已有笔记的摘要或首段，确认相关性
